...
export function MoveToFolderDialog({
  ...
  const moveNotebook = useMoveNotebookToFolder();
  ...
  const handleMove = async () => {
    ...
    await moveNotebook.mutateAsync({ notebookId, folderId: selected });
    onOpenChange(false);
  };
      ...
      <DialogContent className="max-w-md rounded-xl">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold truncate">
            Mover &ldquo;{notebookTitle}&rdquo; para...
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            onClick={handleMove}
            disabled={moveNotebook.isPending}
          ...
          >
            {moveNotebook.isPending && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
            Mover
          </button>
        </div>
      </DialogContent>
