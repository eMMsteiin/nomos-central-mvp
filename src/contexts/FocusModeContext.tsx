...
export function FocusModeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        emitFocusEvent('NOMOS_FOCUS_STOP', { reason: 'completed' });
  ...
  const endSession = useCallback((reason: 'completed' | 'ended_early') => {
    const newState: FocusState = {
      ...state,
      active: false,
      startedAt: null,
  ...
  const updateBlocklist = useCallback((blocklist: string[]) => {
  ...
  const updateAllowlist = useCallback((allowlist: string[]) => {
  ...
  const setDefaultDuration = useCallback((durationMin: number) => {
